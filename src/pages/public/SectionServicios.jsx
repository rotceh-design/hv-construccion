  ...
  return (
    ...
    >
      <div className="h-24 bg-black relative overflow-hidden border-b-2 border-black">
        {isSlot ? (
          <div className="flex items-center justify-center h-full text-[10px] font-black text-zinc-600 uppercase">Sin Imagen</div>
        ) : (
  ...
  return (
    <div className="animate-fade-in max-w-[1400px] mx-auto py-6">
      ...
      <div className="mb-14 mt-6 border-b-4 border-black pb-8">
        <div className="font-mono text-[11px] text-black font-black bg-[#FFCF40] border-2 border-black px-3 py-1 inline-block shadow-[3px_3px_0px_rgba(0,0,0,1)] tracking-widest mb-6 uppercase">
          /// CATÁLOGO TÉCNICO DE ESPECIALIDADES
        </div>
        <h1 className="font-black text-5xl md:text-7xl uppercase leading-none tracking-tight text-white drop-shadow-[4px_4px_0px_rgba(0,0,0,1)]">
          SERVICIOS<br/><span className="text-[#FFCF40]">ESPECIALIZADOS</span>
        </h1>
      </div>

      {/* SELECTOR DE CATEGORÍAS (Botonera) */}
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-4 mb-16">
        {cats.map(([key, data]) => {
          const isActive = catKey === key;
          return (
            <button 
              key={key} 
              className={`flex flex-col p-4 border-2 border-black transition-all text-left relative overflow-hidden group
                ${isActive 
                  ? 'bg-[#FFCF40] shadow-[4px_4px_0px_rgba(0,0,0,1)] -translate-y-1' 
                  : 'bg-white hover:bg-zinc-100 shadow-[2px_2px_0px_rgba(0,0,0,1)] opacity-90'}`}
              onClick={() => setActiveCat(key)}
            >
              <div className={`mb-3 ${isActive ? 'text-black' : 'text-zinc-400'} group-hover:scale-110 transition-transform`}>
                {CAT_ICON[key] ?? <Wrench size={24}/>}
              </div>
              <div className="font-black text-[13px] uppercase leading-none text-black mb-1">{data.label}</div>
              <div className="font-mono text-[10px] font-black text-zinc-500 uppercase">{data.items.length} Partidas</div>
              {isActive && <div className="absolute top-2 right-2 w-2 h-2 bg-black"></div>}
            </button>
          );
        })}
      </div>

      {/* LISTADO DE SERVICIOS (Contenedor HV) */}
      {catData && (
        <div className="animate-fade-in bg-white border-4 border-black shadow-[10px_10px_0px_#FFCF40] overflow-hidden mb-12">
          ...
          <div className="bg-black text-white p-6 flex items-center gap-6 border-b-4 border-black">
            <div className="w-14 h-14 bg-[#FFCF40] text-black flex items-center justify-center shadow-[4px_4px_0px_rgba(255,207,64,0.3)]">
              {CAT_ICON[catKey] ?? <Wrench size={28}/>}
            </div>
            <div>
              <h2 className="font-black text-3xl uppercase leading-none">{catData.label}</h2>
              <p className="font-mono text-[11px] text-[#FFCF40] tracking-widest mt-1 uppercase">Partidas técnicas certificadas</p>
            </div>
          </div>
            ...
            {catData.items.map(item => {
              ...
              return (
                <div key={item.id} className={`transition-colors ${isExp ? 'bg-zinc-50' : 'bg-white hover:bg-zinc-50'}`}>
                  ...
                  <div className="p-6 flex flex-wrap items-center gap-6 cursor-pointer" onClick={() => setExpandedSvc(isExp ? null : item.id)}>
                    <div className="flex-1 min-w-[280px]">
                      ...
                      <div className="flex items-center gap-4 flex-wrap">
                        ...
                        {subExtra > 0 && <span className="text-[10px] font-black text-green-600 bg-green-50 border border-green-200 px-2 py-0.5">+{fmtUF(subExtra)} UF PERSONALIZACIÓN</span>}
                      </div>
                    </div>
                  ...
                  {/* Detalle del Servicio (Expandido) */}
                  {isExp && (
                    <div className="px-8 pb-10 border-t-2 border-zinc-200 animate-fade-in">
                      <div className="bg-black text-white p-5 my-6 font-bold text-sm leading-relaxed border-l-8 border-[#FFCF40]">
                        {item.desc}
                      </div>
                      ...
                      {subs ? (
                        <div className="mt-8">
                          <div className="font-mono font-black text-[11px] text-black mb-6 flex items-center gap-2 uppercase tracking-widest border-b-2 border-black pb-2">
                            <Zap size={14} className="text-[#FFCF40]" fill="currentColor"/> Configuración de Especificaciones
                          </div>
                          {subs.map(grupo => (
                        ...
                        <div className="text-[11px] font-black text-zinc-400 uppercase tracking-widest italic py-4">
                          Servicio con especificaciones base.
                        </div>
                      )}
                    </div>
                  )}
      ...
      {/* BOTÓN FLOTANTE / FINAL DE CARRITO */}
      {cartCount > 0 && (
        <div className="flex justify-end mt-12 mb-20">
          ...
          >
            VER PRESUPUESTO ({cartCount}) <ArrowRight size={24} strokeWidth={3}/>
          </button>
        </div>
      )}
    </div>
