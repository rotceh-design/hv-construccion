...
const SupervisorLayout = ({ children }) => {
  ...
  const menuItems = [
    { name: 'Panel Principal', path: '/supervisor', icon: <LayoutDashboard size={20} /> },
    { name: 'Obras en Terreno', path: '/supervisor/obras', icon: <Map size={20} /> },
    { name: 'Aprobaciones', path: '/supervisor/aprobaciones', icon: <ClipboardCheck size={20} /> },
  ];
    ...
    <div className="flex h-screen bg-light">
      <aside className="w-64 bg-dark text-white flex-col hidden md:flex">
        <div className="h-16 flex items-center justify-center border-b border-dark-light bg-hazard-stripes bg-opacity-20">
          ...
          <h2 className="text-2xl font-display font-bold text-primary tracking-wider">HV<span className="text-white">SUPERVISIÓN</span></h2>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white shadow-sm flex items-center justify-between px-6 border-b-4 border-primary">
          ...
          <div className="flex items-center gap-4 ml-auto">
            ...
            <span className="text-sm font-semibold text-dark hidden md:block">Supervisor Zonal</span>
          </div>
        </header>
